...
/// Пустой список для тестированию экрана с надписью "Здесь пока ничего нет".
//var listData = <ProductEntity>[];

/// Будем работать с копией данных, чтобы можно было использовать исходный список
/// для режима "Без сортировки".
/// В первом варианте (ListView) список был перезагружаемый для режима "Без сортировки".
/// Поэтому не final. Можно поставить final, так как сейчас используется пакет "grouped_list"
/// для группирования оп категориям.
...
/// В первом варианте (ListView) использовалось эта функция для перестроения списка.
